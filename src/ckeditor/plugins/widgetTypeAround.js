import { getTypeAroundFakeCaretPosition, injectUIIntoWidget, isTypeAroundWidget } from "../utils";
import { PLUGIN_DISABLED_EDITING_ROOT_CLASS, TYPE_AROUND_SELECTION_ATTRIBUTE } from "../vars";
import delete_Delete from "./delete";
import enter_Enter from "./enter";
 ...
 */
export default class widgettypearound_WidgetTypeAround extends plugin_Plugin {
  /**
   * @inheritDoc
   */
    ...
    this._listenToIfEnabled(
      ...
      (evt, data, conversionApi) => {
        ...
        if (this._currentFakeCaretModelElement) {
          const selectedViewElement = conversionApi.mapper.toViewElement(
          ...
          if (selectedViewElement) {
            ...
            writer.removeClass(
              POSSIBLE_INSERTION_POSITIONS.map(positionToWidgetCssClass),
              selectedViewElement
            );
  ...
  _handleArrowKeyPressWhenSelectionNextToAWidget(isForward) {
    ...
    const viewElementNextToSelection = editor.editing.mapper.toViewElement(
  ...
  _enableInsertingParagraphsOnTypingKeystroke() {
    ...
    this._listenToIfEnabled(
      ...
      (evt, domEventData) => {
        ...
        if (
          !keyCodesHandledSomewhereElse.includes(domEventData.keyCode) &&
          !isNonTypingKeystroke(domEventData)
        ) {
  ...
  _enableDeleteIntegration() {
    ...
    this._listenToIfEnabled(
      ...
      (evt, domEventData) => {
        ...
        const typeAroundFakeCaretPosition = getTypeAroundFakeCaretPosition(
          ...
          if (range) {
            ...
            if (!range.isCollapsed) {
              ...
              // so calling Model#deleteContent here manually.
                ...
                const deepestEmptyRangeAncestor = getDeepestEmptyElementAncestor(
                  schema,
                  range.start.parent
                );
