 ...
 */
export default class delete_Delete extends plugin_Plugin {
  /**
   * @inheritDoc
   */
  ...
  init() {
    ...
    const deleteForwardCommand = new deletecommand_DeleteCommand(editor, "forward");
    ...
    this.listenTo(
      ...
      (evt, data) => {
        ...
        editor.execute(
          data.direction == "forward" ? "deleteForward" : "delete",
          deleteCommandParams
        );
      ...
      this.listenTo(viewDocument, "keyup", (evt, data) => {
        if (domSelectionAfterDeletion) {
          ...
          domSelection.collapse(
            domSelectionAfterDeletion.anchorNode,
            domSelectionAfterDeletion.anchorOffset
          );
          domSelection.extend(
            domSelectionAfterDeletion.focusNode,
            domSelectionAfterDeletion.focusOffset
          );
